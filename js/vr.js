import { THREE, scene, camera, renderer, loadTexture, createSphere, showLoading, hideLoading,
         layerMono, layerLeft, layerRight } from './core.js';
...
fetch('https://api.github.com/repos/lucakassab/tour360/contents/media')
 ...
 .then(files=>{
   ...
   sel.selectedIndex=0;
   loadTexture(sel.value, sel.options[0].dataset.name.toLowerCase().includes('_stereo'),
               (tex,isStereo)=>createSphere(tex,isStereo));
 });
document.getElementById('btnLoad').onclick=()=>{
  const opt=sel.options[sel.selectedIndex];
  const stereo=opt.dataset.name.toLowerCase().includes('_stereo');
  loadTexture(opt.value, stereo, (tex)=>createSphere(tex,stereo));
};
